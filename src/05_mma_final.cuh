// 05_mma_final.cuh
// PTX MMA HGEMM combining all previous optimizations plus block swizzle
// for L2 cache locality.
// Builds on 04_mma_pipelining: adds block swizzle for improved L2 reuse.
//
// - Block swizzle (USE_SWIZZLE, BLOCK_STRIDE) for L2 cache locality
// - Fragment double buffering (inherited from 04)
// - Async copy at k-loop midpoint (inherited from 04)
// - Cross-tile fragment prefetch (inherited from 04)
...
__global__ void mma_final_kernel(
    ...
    static void Run(int M, int N, int K, __half alpha,
        ...
        if (!configured) {
            cudaFuncSetAttribute(
                mma_final_kernel<BM, BN, BK, WM, WN, STAGES, USE_SWIZZLE, BLOCK_STRIDE>,
